const tg = window.Telegram.WebApp;
        ...
        const ENDPOINTS = {
            ...
            channelPost: `${API_BASE}/api/channel_post`,
        ...
        async function loadData() {
            ...
            try {
                ...
                if (roleAdminBlock.includes('director')) {
                    showAdminBlock = true;
                    document.getElementById('t-channel-post')?.classList.remove('hidden');
                }
        ...
        const sendChannelPost = () => {
            const message = document.getElementById('channel-post-msg').value;
            if(!message.trim()) return tg.showAlert(t('error_fields'));
            apiRequest(ENDPOINTS.channelPost, { message }, "Успешно отправлено!");
        };
        ...
        function openModal(type) {
            ...
            switch(type) {
                ...
                case 'channel-post':
                    html = `
                        <h2 class="text-xl font-bold mb-6 italic text-green-400">Пост в канал ЖК</h2>
                        <p class="text-[11px] text-slate-300 mb-6">Бот автоматически прикрепит кнопку-ссылку для входа в приложение.</p>
                        <textarea id="channel-post-msg" class="text-sm h-40 mb-4" placeholder="Введите текст поста... Поддерживается HTML-разметка (<b>жирный</b>, <i>курсив</i>)"></textarea>
                        <button onclick="sendChannelPost()" class="w-full py-4 bg-green-600 rounded-2xl font-black uppercase text-[10px] tracking-widest active:bg-green-700 transition-colors">Опубликовать</button>`;
                    break;
        ...
        function openSuperadminEdit(complex, source = 'superadmin') {
            ...
            const saChannelCard = document.getElementById('sa-edit-channel-id').parentElement;
            ...
            saChannelCard.style.display = 'block';
            ...
            if (source === 'tasks') {
                if (isAdmin) {
                    ...
                    saChannelCard.style.display = 'none';
            ...
            if (complex) {
                ...
                document.getElementById('sa-edit-channel-id').value = complex.channel_id || "";
                ...
                document.getElementById('sa-edit-channel-id').value = "";
        ...
        async function saveSuperadminComplex() {
            ...
            try {
                ...
                formData.append('channel_id', document.getElementById('sa-edit-channel-id').value);
